...
FILE * tmp = fopen((outfilename+to_string(chunks)).c_str(),"wb");
for(int i=0;i<=lastpos;i++)
fprintf(tmp,"%lld,",tempinp[i]);
fclose(tmp);
}
...
for(int i=0;i<chunks;i++)
{
    ...
    fptrs[i] = fopen((outfilename+to_string(i+1)).c_str(),"rb");
    if(fptrs[i]==NULL)
    {
