...
def join_tables(tablelist):
    ...
    for table in tables:
        for col in range(len(table["cols_order"])):
            ...
            colnames[table["cols_order"][col]] = table["name"]
            i+=1
    recur(tables,[],newtable["rows"])
    return newtable

def f_count(newtable,tablelist,cmd):
        return len(tablelist)
    
def f_sum(newtable,tablelist,cmd):
    sums = 0
    pos = newtable["cols"][cmd]
    for row in tablelist:
            sums+=row[pos]
    return sums

def f_avg(newtable,tablelist,cmd):
    sums = f_sum(newtable,cmd)
    n = len(tablelist)
    avg = sums/n
    return avg

def f_max(newtable,tablelist,cmd):
    maxs = -inf
    pos = newtable["cols"][cmd]
    for row in tablelist:
...
def f_min(newtable,tablelist,cmd):
    mins = inf
    pos = newtable["cols"][cmd]
    for row in tablelist:
...
def aggr(newtable,tablelist,params):
    cmd = [i.strip() for i in re.split("\(|\)",params) if i.strip()!=""]
    res = 0
    if(cmd[0]=="count"):
        res= f_count(newtable,tablelist,cmd[1])
    elif(cmd[0]=="sum"):
        res= f_sum(newtable,tablelist,cmd[1])
    elif(cmd[0]=="avg"):
        res= f_avg(newtable,tablelist,cmd[1])
    elif(cmd[0]=="max"):
        res= f_max(newtable,tablelist,cmd[1])
    elif(cmd[0]=="min"):
        res= f_min(newtable,tablelist,cmd[1])
    return res
...
def group_by_parse(group_by_part,newtable):
    ...
    for row in newtable["rows"]:
        ...
        try:
            idx = tuple(temp)
            tempdict[idx].append(row)
        except KeyError:
            tempdict[idx] = [row]
    temptab = []
    newtemp = []
    for i in tempdict.keys():
        temptab.append(tempdict[i])
        newtemp.append(tempdict[i][0])
    
    newtable["rows"] = newtemp
    newtable["groups"]=temptab
    newtable["group"] = True
...
def show_table(newtable):
    ...
    for row in newtable["rows"]:
        for els in row:
            if(els==None):
                print("{:20}".format(str("")),end="")
            else:
                print("{:20}".format(str(els)),end="")
        print("")
...
def select_parse(select_part,newtable):
    ...
    if(isaggr):
        ...
        if(newtable["group"]):
            newtable["rows"] = []
            for group in newtable["groups"]:
                temprow=[]
                for j in col_list:
                    temp = re.split("\(|\)",j)[0].strip()
                    try:
                        aggrs[temp]
                        temprow.append(aggr(newtable,group,j))
                    except KeyError:
                        try:
                            ps = newtable["cols"][j]
                            temprow.append(group[0][ps])
                            
                        except KeyError:
                            temprow.append(None)
                newtable["rows"].append(temprow)
        else:
            temprow=[]
            for j in col_list:
                temp = re.split("\(|\)",j)[0].strip()
                try:
                    aggrs[temp]
                    temprow.append(aggr(newtable,newtable["rows"],j))
                except KeyError:
                    try:
                        ps = newtable["cols"][j]
                        temprow.append(newtable["rows"][0][ps])
                            
                    except KeyError:
                        temprow.append(None)
            newtable["rows"] = [temprow]
