...
def check():
    ...
    with open(table["input_file"],"r") as f:
        f.readline()
        table["row_len"] = int(f.tell())
    table["rows"] = int(table["total"]//table["row_len"])
    table["effective_mem"] = int(table["memlimit"]//table["threads"])
    table["chunk_len"] = min(table["effective_mem"]//table["row_len"],table["rows"]//table["threads"])
    table["chunks"] = int(table["rows"]//table["chunk_len"])
    table["thread_len"] = int(table["chunks"]//table["threads"])
...
def get_data(n):
    ...
    size = 0
    temp_chunk = []
    print("reading #{} sublist".format(n+1))
    while True:
        ...
        for length in table["ord"]:
            ...
            pos+=length+2
    f.close()
    print("sorting #{} sublist".format(n+1))
    temp_chunk.sort()
    print("writing #{} to disk".format(n+1))
...
def phase1(cmd):
    ...
    print("Number of Subfiles : {}".format(table["chunks"]))
    threadlist = []
